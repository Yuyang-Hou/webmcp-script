...
test('standard MCP discovery, authentication, invocation and disconnect', async () => {
  ...
  try {
    ...
    assert.deepEqual(tools.map(x=>x.name).sort(),['call_tool','describe_tool','inspect_page','pages','visit_page']);
    ...
    ws.on('message',raw=>{
      ...
      if(req.method==='describe' && req.params.revision!=='r1') ws.send(JSON.stringify({id:req.id,error:{message:'STALE_REVISION'}}));
    ...
    assert.equal(JSON.parse(pages.content[0].text).method,'pages');
