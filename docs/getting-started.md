...
| 另一个浏览器已连接，新浏览器无法连接 | 每个端口只连接一个浏览器；为当前 MCP 服务设置空闲的 WEBMCP_PORT，重新启动服务并调用 connection_info 获取连接码，不必关闭另一个浏览器 |
| The extensions gallery cannot be scripted | Chrome 扩展商店禁止注入；这不是配对码错误，修复版会跳过商店页面 |
